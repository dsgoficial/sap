...
CREATE TABLE macrocontrole.etapa(--fixme como lidar com situações de criar mais revisoes e correcoes para uma determinada unidade de trabalho?
	id SERIAL NOT NULL PRIMARY KEY,
...
CREATE TABLE macrocontrole.requisito_finalizacao(
	...
	subfase_id INTEGER NOT NULL REFERENCES macrocontrole.subfase (id) -- FIXME como lidar com configurações? deixar fixo pela linha de produção?
);
...
CREATE TABLE macrocontrole.perfil_propriedades_camada( --fixme tirar nome perfil
	id SERIAL NOT NULL PRIMARY KEY,
	camada_id INTEGER NOT NULL REFERENCES macrocontrole.camada (id),
...
CREATE TABLE macrocontrole.restricao_etapa( --fixme repensar restricao_etapa
	id SERIAL NOT NULL PRIMARY KEY,
...
CREATE TABLE macrocontrole.lote( --FIXME separar para lotes de unidade de trabalho
	id SERIAL NOT NULL PRIMARY KEY,
...
CREATE TABLE macrocontrole.unidade_trabalho(
	...
	dado_producao_id INTEGER NOT NULL REFERENCES macrocontrole.dado_producao (id),
 	subfase_id INTEGER NOT NULL REFERENCES macrocontrole.subfase (id),
	lote_id INTEGER NOT NULL REFERENCES macrocontrole.lote (id), --FIXME link com produto ou projeto?
...
CREATE TABLE macrocontrole.perfil_producao_operador(
	id SERIAL NOT NULL PRIMARY KEY,
  	usuario_id INTEGER NOT NULL REFERENCES dgeo.usuario (id), --FIXME linkar com projeto?
	perfil_producao_id INTEGER NOT NULL REFERENCES macrocontrole.perfil_producao (id),
	UNIQUE (usuario_id)
);
