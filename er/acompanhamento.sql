...
$$
  ...
  BEGIN
    ...
    IF num > 0 THEN
      ...
      SELECT ut.id, ut.lote_id, ut.subfase_id, ut.disponivel, rest.id IS NOT NULL AS restrito, l.nome AS bloco, ut.nome, ut.dificuldade, dp.configuracao_producao AS dado_producao, dp.configuracao_producao, tdp.nome AS tipo_dado_producao, ut.prioridade, ut.geom';
      ...
      rules_txt := rules_txt || '<rule symbol="' ||  iterator || '" key="{' || uuid_generate_v4() ||'}" label="restrição" filter="restrito IS TRUE"/>';
