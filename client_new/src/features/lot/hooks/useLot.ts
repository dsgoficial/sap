// Path: features\lot\hooks\useLot.ts
import { useQuery } from '@tanstack/react-query';
import { getLots } from '../../../services/lotService';
import { LotTableData } from '../types';
    ...
    select: (data) => {
      ...
      data.dados.forEach(element => {
        ...
        if (element.month >= 1 && element.month <= 12) {
          tableData[element.lote][element.subfase][element.month - 1] = element.count;
        }
      });
      ...
      const result: LotTableData[] = Object.keys(tableData).map(lotKey => {
        return {
          lot: lotKey,
          rows: Object.keys(tableData[lotKey]).map(subphaseKey => {
            ...
            MONTHS.forEach((month, idx) => {
              ...
              row[month.id] = idx > currentMonthIdx ? '-' : tableData[lotKey][subphaseKey][idx];
            });
