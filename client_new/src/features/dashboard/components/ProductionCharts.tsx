...
export const ProductionCharts = ({ data, isLoading }: ProductionChartsProps) => {
  ...
  const pieData = data ? [
    ...
    { 
      label: 'Não Iniciado', 
      value: data.summary.totalProducts - (data.summary.completedProducts + data.summary.runningProducts),
      color: '#F24C3D'
    },
    { 
      label: 'Em Execução', 
  ...
  const getMonthlyChartSeries = () => {
    ...
    const lotKeys = Object.keys(data.monthlyData[0])
      .filter(key => key.startsWith('lot_'));
