...
managePermissions.grantPermissionsUser = async (
  ...
  await connection.tx(async t => {
    ...
    await t.none(`GRANT CONNECT ON DATABASE $<db:name> TO $<login:name>;`, {
      dbName,
      login
    });
    ...
    if (tipoEtapa === 3) {
      ...
      const camadasApontamentoSQL = grantInfo
        ...
        .map(
          v =>
            `GRANT UPDATE(${v.atributo_justificativa_apontamento}, ${v.atributo_situacao_correcao}) ON ${v.schema}.${v.nome_camada} TO ${login};`
        )
        .filter((v, i, array) => array.indexOf(v) === i)
    ...
    await t.none(
      `GRANT SELECT ON ALL TABLES IN SCHEMA public TO $<login:name>;`,
      { login }
    );
